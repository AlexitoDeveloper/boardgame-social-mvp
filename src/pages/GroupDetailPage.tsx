...
import { ArrowLeft, Users, Layers, Calendar, Trophy, Dices, Award } from 'lucide-react'
...
import { GroupPastMatchesList } from '../components/group-detail/GroupPastMatchesList'
...
export function GroupDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'meetups' | 'matches' | 'ludoteca' | 'hall_of_fame' | 'table_tools' | 'members'>('meetups')
  ...
  const tabOptions = [
    { id: 'meetups' as const, label: t('groups.upcomingMeetupsTab', 'Quedadas'), icon: Calendar, count: hub.upcomingMeetups.length > 0 ? hub.upcomingMeetups.length : undefined },
    { id: 'matches' as const, label: t('groups.pastMatchesSubTab', 'Partidas'), icon: Trophy, count: hub.pastMeetups.length > 0 ? hub.pastMeetups.length : undefined },
    { id: 'ludoteca' as const, label: t('groups.sharedLudoteca', 'Ludoteca'), icon: Layers, count: hub.mergedCollection.length },
    { id: 'hall_of_fame' as const, label: t('groups.hallOfFame', 'Salón de la Fama'), icon: Award },
  ...
  return (
    <section className="space-y-6 pb-20 max-w-6xl mx-auto">
      <GroupHeroHeader
        ...
        onOpenQrModal={() => hub.setIsQrModalOpen(true)} onOpenQuickLogModal={() => navigate(`/partida/nueva?groupId=${groupId}`)}
      ...
      <div className="min-h-[300px]">
        {activeTab === 'meetups' && groupId && (
          <GroupUpcomingMeetups
            groupId={groupId}
        ...
        {activeTab === 'matches' && groupId && (
          <GroupPastMatchesList
            groupId={groupId}
      ...
      <AddGameToLibraryModal
        ...
        groupMembers={[
          ...
          ...hub.guests.filter(g => !g.associated_user_id).map(g => ({
            id: g.id,
            name: g.name,
