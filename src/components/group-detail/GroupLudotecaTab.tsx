        ...
        <div className="flex items-center gap-2.5 flex-1 max-w-lg">
          ...
          {onOpenAddGame && (
            <Button
              ...
              className="h-9 px-3 rounded-xl font-bold text-xs gap-1.5 shrink-0 shadow-xs cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>{t('groups.addGameToGroup', 'Añadir Juego')}</span>
            </Button>
          )}
        </div>

        <div className="text-xs text-muted-foreground font-semibold flex items-center gap-1 shrink-0">
        ...
        <div className="text-center py-16 border border-dashed border-border/50 rounded-2xl bg-muted/15 max-w-md mx-auto space-y-3 p-6">
          ...
          {onOpenAddGame && (
            <Button
              size="sm"
              onClick={onOpenAddGame}
              className="mt-2 rounded-xl font-bold text-xs gap-1.5 shadow-sm cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>{t('groups.addGameToGroup', 'Añadir Juego')}</span>
            </Button>
