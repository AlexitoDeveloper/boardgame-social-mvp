...
import { Dices, Download, Plus, Search } from 'lucide-react'
...
export function CollectionTab({
  ...
  const toggleWantToPlay = (e: React.MouseEvent, bggId: number) => {
    e.preventDefault()
    e.stopPropagation()
    const next = wantToPlayIds.includes(bggId)
  ...
  const playedBggIds = useMemo(() => {
    ...
    meetups.filter(m => m.completed).forEach(m => {
      (m.games || []).forEach(g => ids.add(g.bgg_id))
    })
    return ids
  }, [meetups])

  const { filteredGames, counts } = useMemo(() => {
    const unplayed = collectionGames.filter(g => !playedBggIds.has(g.bgg_id))
    const wantToPlay = collectionGames.filter(g => wantToPlayIds.includes(g.bgg_id))
  ...
  const filterChips: { id: FilterType; label: string; count: number }[] = [
    ...
    { id: 'unplayed', label: t('profile.collection.filterUnplayed', 'Shelf of Shame'), count: counts.unplayed }
  ]
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center justify-between">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            ...
            className="pl-8.5 h-9 text-xs rounded-xl bg-card/50"
          />
        </div>
      ...
      {loadingCollection ? (
        ...
        <div className="text-center py-12 px-4 bg-card/40 rounded-2xl border border-dashed border-border/40 space-y-3 select-none">
          ...
          <p className="text-sm font-bold text-muted-foreground">
            {searchQuery ? 'No hay juegos que coincidan con la búsqueda' : t('profile.collection.emptyTitle')}
          </p>
          {isOwnProfileEditable && (
